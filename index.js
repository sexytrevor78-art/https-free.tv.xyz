...
builder.defineMetaHandler(async ({ id }) => {
  ...
  if (!channels[id]) {
    console.warn('[RED-DRAGON] Meta requested for unknown ID:', id);
    return { meta: null };
  }

  const ch = channels[id];
  console.log('[RED-DRAGON] Serving meta for:', ch.name);
  ...
  return {
    meta: {
      ...
      logo: ch.logo || '',
      background: ch.logo || '',
      description: `Live channel: ${ch.name}`
    }
  };
});
